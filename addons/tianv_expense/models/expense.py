...
class HrExpense(osv.Model):
    ...
    def action_move_create(self, cr, uid, ids, context=None):
        ...
        for exp in self.browse(cr, uid, ids, context=context):
            ...
            if not exp.employee_id.address_id.property_account_payable.id:
            ...
            acc = exp.employee_id.address_id.property_account_payable.id
            eml.append({
                'type': 'dest',
                'name': '/',
                'price': total,
            ...
            lines = map(lambda x: (0, 0, self.line_get_convert(cr, uid, x, exp.employee_id.address_id, exp.date_confirm, context=context)), eml)
